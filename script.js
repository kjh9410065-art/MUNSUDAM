...
function buildPrintTemplate(doc, values) {
  ...
  if (doc.id === "receipt") return `<h1>영 수 증</h1><div class="receipt"><div class="receipt-row"><span>받는 사람</span><strong>${val("recipient") || "________________"}</strong></div><div class="receipt-amount">${values.amount ? Number(values.amount).toLocaleString("ko-KR")+" 원" : "____________ 원"}</div><div class="receipt-row"><span>지급자</span><strong>${val("payer") || "________________"}</strong></div><div class="receipt-row"><span>내용</span><strong>${val("content") || "________________"}</strong></div></div><div class="date-line">${date(values.date) || todayText}</div><div class="signature-line">받는 사람 : ${val("recipient") || "________________"} &nbsp;&nbsp; (서명 또는 인)</div>`;
