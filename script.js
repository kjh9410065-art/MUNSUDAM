...
function buildPrintTemplate(doc, values) {
  ...
  if (doc.id === "agreement") { const template = `<h1>각 서</h1><div class="info">${infoRow("작 성 자",val("writer"))}${infoRow("제 목",val("title"))}</div>${contentBox("각서 내용",val("content"),"32mm")}<div class="confirmation">위 내용을 확인하고 성실히 이행할 것을 확인합니다.</div><div class="date-line">${date(values.date) || todayText}</div><div class="signature-line">작성자 : ${val("writer") || "________________"} &nbsp;&nbsp; (서명 또는 인)</div>`; return duplicateHalfPage(template); }
  if (doc.id === "consent") { const template = `<h1>동 의 서</h1><div class="info">${infoRow("성 명",val("name"))}${infoRow("동의 사항",val("subject"))}</div>${contentBox("동의 내용",val("content"),"32mm")}<div class="confirmation">본인은 위 내용을 확인하고 이에 동의합니다.</div><div class="date-line">${date(values.date) || todayText}</div><div class="signature-line">성 명 : ${val("name") || "________________"} &nbsp;&nbsp; (서명 또는 인)</div>`; return duplicateHalfPage(template); }
